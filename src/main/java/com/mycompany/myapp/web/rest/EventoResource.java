...
public class EventoResource {
    ...
    public ResponseEntity<Evento> updateEvento(@RequestBody Evento evento) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, evento.getId().toString()))
            .body(result);
    }
    ...
    @GetMapping("/eventospersona/{id}")
    public List<Evento> getEventopersona(@PathVariable Long id) {
        log.debug("REST request to get Evento : {}", id);
        List<Evento> evento = eventoRepository.findAlleventospersona(id);
        return evento;
    }
    /**
